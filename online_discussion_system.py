...
from utils import get_docking_context, get_activity_cliff_summary, draw_highlighted_pair
...
3. 일관된 '-입니다' 체로 작성하되, 가설이므로 확정적 단언은 지양하세요."""
    ...
    def generate(self, shared_context: Dict[str, Any]) -> Dict[str, Any]:
        """생체분자 상호작용 관점의 가설 생성 (공통 도킹 결과 활용)"""
        # Phase 1에서 이미 수행된 도킹 결과를 사용
        docking_results = shared_context.get('literature_context') or shared_context.get('docking_results')
    ...
    def _perform_docking_analysis(self, shared_context: Dict[str, Any]) -> Dict:
        ...
        target_name = shared_context.get('target_name', '6G6K')
        ...
        if high_active.get('smiles') and low_active.get('smiles'):
            ...
            docking_results = get_docking_context(high_active.get('smiles'), low_active.get('smiles'), target_name)
    ...
    def _build_interaction_prompt(self, shared_context: Dict[str, Any], docking_results: Dict = None) -> str:
        ...
        **결과 형식 (반드시 이 형식을 정확히 따르세요):**
        ...
        상세 분석:
        1. 단백질-리간드 결합: [활성 부위와의 결합 방식 차이, 검출된 잔기와의 상호작용 변화]
        2. 상호작용 패턴: [수소결합, 소수성 상호작용, π-π 스택킹 등의 변화와 활성 영향]
        3. 결합 기하학: [분자 형태 변화가 단백질 포켓과의 입체적 적합성에 미치는 영향]
        4. 약리학적 메커니즘: [결합 친화도 변화가 기능적 활성 변화로 이어지는 과정]
        5. ADMET 영향: [구조 변화가 대사 안정성, 선택성, 투과성에 미치는 영향]
...
def display_expert_result(result: Dict):
    ...
    with st.expander(f"{result['agent_name']} 결과", expanded=True):
        ...
        # 각 화합물별 상호작용 정보 표시 (반데르발스 접촉 포함, 잔기별 해석 강화)
        def show_interactions(result, label):
            st.write(label)
            fingerprint = result.get('interaction_fingerprint', {})
            for key, name in interaction_names.items():
                interactions = fingerprint.get(key, [])
                if interactions:
                    if key == 'VdWContact':
                        # 잔기별 해석 예시: 각 잔기의 역할을 간단히 설명
                        details = ', '.join(f"{res} (구조적 안정성 기여)" for res in interactions)
                        st.write(f"{name}: {details}")
                    else:
                        st.write(f"{name}: {', '.join(str(x) for x in interactions)}")
                else:
                    st.write(f"{name}: 없음")
        with col1:
            st.write("**생성된 가설:**")
...
def run_online_discussion_system(selected_cliff: Dict, target_name: str, api_key: str, llm_provider: str = "OpenAI", cell_line: str = None) -> Dict:
    ...
    if cliff_summary:
        ...
        # 도킹 결과를 shared_context에 추가
        shared_context['literature_context'] = docking_results
        shared_context['docking_results'] = docking_results
        shared_context['context_quality'] = 'Enhanced' if docking_results else 'Basic'
        shared_context['evidence_level'] = 'Docking-backed' if docking_results else 'Data-only'
        ...
        with st.expander("도킹 시뮬레이션 결과", expanded=False):
            ...
            with col1:
                ...
                st.markdown(f"- **반데르발스접촉:** {', '.join(docking1['interaction_fingerprint']['VdWContact']) if docking1['interaction_fingerprint']['VdWContact'] else '없음'}")
            ...
            with col2:
                ...
                st.markdown(f"- **반데르발스접촉:** {', '.join(docking2['interaction_fingerprint']['VdWContact']) if docking2['interaction_fingerprint']['VdWContact'] else '없음'}")
...
def display_docking_results(docking_analysis: dict, agent_name: str):
    """실제 도킹 시뮬레이션 결과와 예시 데이터를 모두 지원하는 종합 표시 함수"""
    if not docking_analysis:
        return

    # 실제 데이터인지 확인
    is_real_data = docking_analysis.get('_metadata', {}).get('data_source') == 'real_docking_pipeline'
    
    # 기존 expander 구조 유지
    with st.expander("🧬 도킹 시뮬레이션 분석 결과", expanded=False):
        
        # 데이터 소스 표시 (새로 추가)
        if is_real_data:
            st.success("✅ **실제 AutoDock Vina + ProLIF 분석 결과**")
        else:
            st.info("ℹ️ **예시 데이터 (도킹 파이프라인 미사용)**")
        
        # 기존 구조 유지: high_active_docking, low_active_docking 형식 우선 지원
        ...
        elif 'compound1' in docking_analysis and 'compound2' in docking_analysis:
            # 새로운 형식도 지원 (실제 도킹 파이프라인 결과)
            high_result = docking_analysis['compound1']
            low_result = docking_analysis['compound2']
        else:
            st.warning("도킹 결과 형식을 인식할 수 없습니다.")
            return

        # 기존 1) 결합 친화도 분석 구조 유지
        st.markdown("**1) 결합 친화도 분석**")
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            st.write("고활성 화합물")
            # 기존 binding_affinity와 새로운 binding_affinity_kcal_mol 모두 지원
            affinity1 = high_result.get('binding_affinity', high_result.get('binding_affinity_kcal_mol', 0))
            st.write(f"• 결합 친화도: {affinity1:.1f} kcal/mol")
        
        with col2:
            st.write("저활성 화합물")
            affinity2 = low_result.get('binding_affinity', low_result.get('binding_affinity_kcal_mol', 0))
            st.write(f"• 결합 친화도: {affinity2:.1f} kcal/mol")

        # 기존 comparative_analysis 구조 유지하되 계산도 지원
        with col3:
            st.write("친화도 차이")
            if 'comparative_analysis' in docking_analysis:
                ...
                # 직접 계산
                diff_value = affinity1 - affinity2
        ...
        with col4:
            ...
            if 'comparative_analysis' in docking_analysis:
                ...
                # 계산된 값으로 지원 여부 판단
                supports_cliff = diff_value < 0  # 고활성이 더 강한 결합을 보이면 지지
                st.write(f"• 활성비: {abs(diff_value/max(abs(diff_value), 0.1)):.1f}배")
            st.write(f"• 실험 일치: {'예' if supports_cliff else '아니오'}")

        # 기존 2) 단백질-리간드 상호작용 구조 유지
        st.markdown("**2) 단백질-리간드 상호작용 패턴**")
        
        # 기존 interaction_names 딕셔너리 유지하되 확장
        interaction_names = {
            'Hydrogenbonds': '🔵 수소결합',
        ...
        col1, col2 = st.columns(2)
        # 각 상호작용 타입별 잔기 해석 예시 사전
        interaction_role_examples = {
            'Hydrogenbonds': '결합 특이성 강화',
            'Hydrophobic': '소수성 포켓 안정화',
            'VdWContact': '구조적 안정성 기여',
            'Saltbridges': '정전기적 결합 기여',
            'Halogenbonds': '할로겐 상호작용 통한 결합 방향성',
            'Waterbridges': '물 분자 매개 결합',
            'hydrogen_bonds': '결합 특이성 강화',
            'hydrophobic': '소수성 포켓 안정화',
            'pi_stacking': 'π-π 적층 통한 결합 강화',
            'electrostatic': '정전기적 결합 기여'
        }
        def show_interactions(result, label):
            st.write(label)
            fingerprint = result.get('interaction_fingerprint', {})
            for key, name in interaction_names.items():
                interactions = fingerprint.get(key, [])
                if interactions:
                    role = interaction_role_examples.get(key, '')
                    details = ', '.join(f"{res}{f' ({role})' if role else ''}" for res in interactions)
                    st.write(f"{name}: {details}")
                else:
                    st.write(f"{name}: 없음")
        with col1:
            show_interactions(high_result, f"화합물 1 (고활성, ID: {high_result.get('id', high_result.get('smiles', '')[:8])})")
        with col2:
            show_interactions(low_result, f"화합물 2 (저활성, ID: {low_result.get('id', low_result.get('smiles', '')[:8])})")
        
        # 실제 데이터일 경우에만 추가 정보 표시 (새로 추가)
        if is_real_data:
            st.markdown("**3) 실험 상세 정보**")
            metadata = docking_analysis.get('_metadata', {})
            col1, col2, col3 = st.columns(3)
            with col1:
                st.write(f"📊 **도킹 소프트웨어:** {metadata.get('vina_version', 'AutoDock Vina')}")
            with col2:
                st.write(f"🔬 **상호작용 분석:** ProLIF")
            with col3:
                import time
                st.write(f"⏰ **분석 시간:** {time.strftime('%H:%M:%S', time.localtime(metadata.get('timestamp', time.time())))}")

        # 기존 종합 해석 부분도 유지하되 조건부로 표시
        if 'comparative_analysis' in docking_analysis:
            st.markdown("**3) 도킹 분석 종합 해석**" if not is_real_data else "**4) 도킹 분석 종합 해석**")
            comp_analysis = docking_analysis['comparative_analysis']
            diff_value = comp_analysis['affinity_difference']
            supports_cliff = comp_analysis.get('supports_activity_cliff', False)
            ...
            # 기존 추가 분석 제안도 유지
            st.markdown("**4) 후속 분석 제안**" if not is_real_data else "**5) 후속 분석 제안**")
            suggestions = [
...
def display_interaction_list(interactions, interaction_names):
    """기존 상호작용 리스트 표시 함수 - ProLIF 결과도 지원하도록 확장"""
    if not interactions:
        st.write("• 검출된 상호작용 없음")
        return
    
    for interaction_type, residues in interactions.items():
        if residues:  # 빈 리스트가 아닌 경우만
            interaction_name = interaction_names.get(interaction_type, interaction_type)
            
            # 리스트와 불린 값 모두 지원 (ProLIF 결과 대응)
            if isinstance(residues, list):
                residue_text = ', '.join(residues[:4])  # 최대 4개만 표시
                if len(residues) > 4:
                    residue_text += f" 외 {len(residues)-4}개"
            elif isinstance(residues, bool) and residues:
                # ProLIF 결과에서 True인 경우 (잔기 정보가 interaction_type에 포함됨)
                residue_text = interaction_type.replace('_', ' ').title()
            else:
                continue  # False이거나 다른 타입인 경우 건너뛰기
